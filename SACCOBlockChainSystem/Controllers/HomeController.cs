...
namespace SACCOBlockChainSystem.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        // In HomeController.cs - Add these methods to populate the new data
        ...
        private async Task<ContributionStats> GetContributionStatisticsAsync()
        {
            ...
            try
            {
                ...
                for (int month = 1; month <= 12; month++)
                {
                    ...
                    stats.ContributionByMonth.Add(DateTimeFormatInfo.CurrentInfo.GetMonthName(month), monthContributions);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task<LoanPerformanceStats> GetLoanPerformanceStatsAsync()
        {
            ...
            try
            {
                ...
                {
                    var statusName = status.Status switch
                    {
                        0 => "Pending",
                        1 => "Approved",
                        2 => "Disbursed",
                    ...
                    stats.LoansByStatus[statusName] = status.Count;
                }

                // Calculate average loan amount
                stats.AverageLoanAmount = await _context.Loans
                    .Where(l => l.LoanAmt.HasValue)
                    .AverageAsync(l => l.LoanAmt ?? 0);

                // Calculate average repayment period
                //stats.AverageRepaymentPeriod = await _context.Loans
                //    .Where(l => l.RepayPeriod.HasValue)
                //    .AverageAsync(l => l.RepayPeriod ?? 0);

                // Calculate repayment rate (simplified)
                var totalLoans = await _context.Loans.CountAsync(l => l.Status == 1);
                var repaidLoans = await _context.Loans
                    .Join(_context.Loanbals,
                        l => l.LoanNo,
                        lb => lb.LoanNo,
                        (l, lb) => new { Loan = l, LoanBal = lb })
                    .CountAsync(x => x.LoanBal.Cleared);

                stats.RepaymentRate = totalLoans > 0 ? (repaidLoans * 100m / totalLoans) : 0;

                // Default rate (simplified)
                stats.DefaultRate = 100m - stats.RepaymentRate;
            }
            catch (Exception ex)
            {
            ...
            return new List<AgeGroupData>
    {
        ...
        // Update the Index method to include these new data calls:
            ...
            try
            {
                ...
                for (int i = 0; i < months; i++)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        // Add zero data for this month if there's an error
                        data.Add(new MonthlyTransactionData
                        {
                            Month = monthName,
                            Deposits = 0,
            ...
            try
            {
                ...
                for (int i = 0; i < months; i++)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        // Add zero data for this period if there's an error
                        data.Add(new MemberGrowthData
                        {
                            Period = monthName,
                            NewMembers = 0,
        ...
        // Sample data generators (for testing/demo purposes)
        ...
        private List<MemberGrowthData> GenerateSampleMemberGrowthData(int months)
        {
            ...
            for (int i = 0; i < months; i++)
            {
                ...
                cumulativeNewMembers += newMembersThisMonth;
                cumulativeTotalMembers += newMembersThisMonth;

                // Add some variation
                newMembersThisMonth = Math.Max(0, newMembersThisMonth + random.Next(-2, 3));
                cumulativeTotalMembers = Math.Max(cumulativeTotalMembers, data.LastOrDefault()?.TotalMembers ?? 50);
        ...
        private async Task<DashboardVM> GetUniversalDashboardDataAsync()
        {
            ...
            try
            {
                ...
                // Blockchain stats - ALL users can see
        ...
        public async Task<IActionResult> GetChartData()
        {
            try
            {
                var monthlyTransactions = await _dashboardService.GetMonthlyTransactionsAsync(6);
                var memberGrowth = await _dashboardService.GetMemberGrowthAsync(12);
